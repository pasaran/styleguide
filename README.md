...
В частности:
  ...
  * Всегда отбивать пробелами изнутри круглые скобки:
    ...
    (более мягкий вариант, если есть рядом стоящие '((' или '))'.
  ...
  * Использовать `under_score_style` для всех переменных и функций.
    ...
    Да, подчеркивание непривычно. Но зато решает задачу читаемости.
...
Между `do` и `{` — пробел, между `}` и `while` — пробел,
...
Между `switch` и `(` — пробел, между `)` и `{` — пробел.
...
Пробелы после `(` и перед `)` не нужны.
