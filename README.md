...
Это мой личный styleguide, продиктованный моим личным чувством прекрасного (и ужасного)
и зоркостью моих глаз (точнее, отсутствием оной).

## Основное положение

Экономить байты в коде должен обфускатор,
а код должен быть максимально читаемым и понятным.


## Читаемость

В частности, я не экономлю пустые строки и пробелы, отделяю все сколько-нибудь
отдельные блоки кода пустыми строками, части выражений — пробелами.

Еще один важный момент: diff-friendly.
Т.е. нужно минимизировать количество "мусорных" diff'ов.
...
Именно поэтому я стараюсь писать код так, чтобы добавление, удаление, перенос строчек кода,
не приводило бы к изменениям других строчек кода.
Каждая строчка — независимо от соседей.
...
Перед открывающей `{` всегда ставится пробел, а после — перевод строки.
Однострочные блоки кода всегда запрещены:
...
Везде, где синтаксис JavaScript'а разрешает использовать как блок кода `{ ... }`,
так и одиночное выражение, везде нужно использовать полноценный блок,
причем, не однострочный:
...
Каждый блок, кроме последнего, заканчивается пустой строкой.
...
Всегда используем инлайновые комментарии, а не блочные.
...
В целом же я противник jsdocs — не рассматриваю их как комментарии.
...
Я не использую никаких IDE или других инструментов, умеющих извлекать пользу из jsdocs,
поэтому: никаких jsdocs в моем коде! :)
...
Если у блока есть побочные эффекты, то всегда используем `if`.

Тернарный оператор, `&&` и `||` могут использоваться как часть выражения, возвращаюшего значение
и не имеющего side effects:
...
Следует избегать принудительной смены контекста при вызове функций.
